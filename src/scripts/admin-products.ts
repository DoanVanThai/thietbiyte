...
// Pricing mode.
const priceInput = document.querySelector<HTMLElement>("[data-price-input]");
document.querySelectorAll<HTMLInputElement>('input[name="priceMode"]').forEach((input) => input.addEventListener("change", () => { if (priceInput) priceInput.hidden = input.value !== "show"; }));
