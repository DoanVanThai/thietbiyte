...
if (root) {
  ...
  upload?.addEventListener("change",async()=>{const file=upload.files?.[0];if(!file)return;const body=new FormData();body.set("file",file);if(feedback)feedback.textContent="Đang tải ảnh…";const response=await fetch("/api/admin/upload",{method:"POST",body});const result=await response.json();if(response.ok&&field("image")){field("image")!.value=result.url;if(feedback)feedback.textContent="Đã tải ảnh.";}else if(feedback)feedback.textContent=result.error||"Tải ảnh thất bại.";});
