...
const bodyHeadingFontSize = 12.8;
const tableLabelFontSize = 11.5;
...
const drawFirstPageHeader = (doc: PDFKit.PDFDocument, input: SalesQuotePdfInput, company: QuoteCompanyDetails, logo: Buffer) => {
  ...
  doc.fontSize(11.5).text(`Số: ${clean(input.quoteNumber)}`, { align: "center" });
  doc.fillColor(ink).font("Italic").fontSize(bodyFontSize).text(dateCopy(input.quoteDate, input.city), margin, doc.y + 14, { width: contentWidth, align: "right" });
  doc.fillColor(ink).font("Bold").fontSize(bodyHeadingFontSize).text(`Kính gửi: ${clean(input.customer.organization || input.customer.name).toLocaleUpperCase("vi")}`, margin, doc.y + 20, { width: contentWidth });
  doc.font("Regular").fontSize(bodyFontSize);
  ...
  doc.font("Italic").fontSize(bodyFontSize).text(clean(input.introduction), margin, doc.y + 8, { width: contentWidth, align: "justify", lineGap: 2 });
  doc.y += 12;
};
...
const drawTableHeader = (doc: PDFKit.PDFDocument) => {
  ...
  doc.fillColor(ink).font("Bold").fontSize(tableLabelFontSize);
  doc.text("STT", margin, y + 14, { width: indexWidth, align: "center" });
  doc.text("TÊN SẢN PHẨM VÀ CẤU HÌNH", margin + indexWidth, y + 14, { width: nameWidth, align: "center" });
  doc.text("ĐƠN GIÁ\n(VNĐ)", margin + indexWidth + nameWidth, y + 7, { width: priceWidth, align: "center", lineGap: 0 });
  doc.y = y + height;
};
...
const paragraphStyle = (copy: string, paragraphIndex: number) => {
  ...
  if (paragraphIndex === 0) return { font: "Bold", size: 13.2, lineGap: 2.2, indent: 0 };
  if (heading) return { font: "BoldItalic", size: bodyHeadingFontSize, lineGap: 2, indent: 0 };
  if (copy.startsWith("- ")) return { font: "Regular", size: bodyFontSize, lineGap: 2, indent: 10 };
  return { font: "Regular", size: bodyFontSize, lineGap: 2, indent: 0 };
};
...
const drawRowLabels = (doc: PDFKit.PDFDocument, item: ResolvedQuoteItem, index: number, top: number, bottom: number, continued: boolean) => {
  doc.fillColor(ink).font("Bold").fontSize(tableLabelFontSize).text(continued ? `${index + 1}\n(tiếp)` : String(index + 1), margin + 2, top + 9, { width: indexWidth - 4, align: "center", lineGap: 1 });
  const priceBlockHeight = !continued && item.quantity > 1 ? 31 : 15;
  const priceY = top + Math.max(8, (bottom - top - priceBlockHeight) / 2);
  doc.fillColor(ink).font("Bold").fontSize(tableLabelFontSize).text(money(item.unitPrice), margin + indexWidth + nameWidth + 5, priceY, { width: priceWidth - 10, align: "center" });
  if (!continued && item.quantity > 1) doc.fillColor(muted).font("Italic").fontSize(10).text(`SL: ${item.quantity}`, margin + indexWidth + nameWidth + 5, priceY + 17, { width: priceWidth - 10, align: "center" });
};
...
const drawProduct = (doc: PDFKit.PDFDocument, input: SalesQuotePdfInput, item: ResolvedQuoteItem, index: number) => {
  ...
  paragraphs.forEach((paragraph, paragraphIndex) => {
    ...
    const style = item.descriptionRich?.paragraphs?.length
      ? { font: "Regular", size: paragraphIndex === 0 ? 13.2 : bodyFontSize, lineGap: 2, indent: copy.startsWith("- ") ? 10 : 0 }
    ...
    if (contentY + height + 20 > footerTop) continueOnNextPage();
  ...
  if (item.quantity > 1) {
    ...
    doc.fillColor(muted).font("Italic").fontSize(10.5).text(`Thành tiền: ${item.quantity} x ${money(item.unitPrice)} VNĐ`, margin, doc.y, { width: contentWidth - 145, align: "right", lineBreak: false });
...
const drawTerms = (doc: PDFKit.PDFDocument, input: SalesQuotePdfInput, grandTotal: number, company: QuoteCompanyDetails) => {
  ...
  doc.font("Italic").fontSize(bodyFontSize);
  ...
  doc.fillColor(primary).font("Bold").fontSize(bodyHeadingFontSize).text("TỔNG GIÁ TRỊ", margin + 10, grandTotalY + 10, { width: contentWidth - 190, lineBreak: false });
  doc.fillColor(ink).fontSize(13.5).text(`${money(grandTotal)} VNĐ`, margin + contentWidth - 180, grandTotalY + 8, { width: 170, align: "right", lineBreak: false });
  doc.fillColor(ink).font("Italic").fontSize(bodyFontSize).text(amountInWords, margin + 10, grandTotalY + 42, { width: contentWidth - 20, lineGap: 2 });
  doc.y += 14;
  const terms = [
  ...
  doc.fillColor(ink).font("BoldItalic").fontSize(bodyHeadingFontSize).text("ĐIỀU KHOẢN THƯƠNG MẠI", margin, termsTitleY, { width: contentWidth });
  doc.strokeColor("#000000").lineWidth(0.8).moveTo(margin, termsTitleY + 13).lineTo(margin + 142, termsTitleY + 13).stroke();
  doc.y = termsTitleY + 21;
  doc.font("Italic").fontSize(bodyFontSize);
  doc.text(clean(terms), margin, doc.y, { width: contentWidth, lineGap: 2.5 });
  doc.y += 24;
  const signatureY = doc.y;
  doc.font("Bold").fontSize(bodyFontSize).fillColor(ink).text("ĐẠI DIỆN KHÁCH HÀNG", margin, signatureY, { width: contentWidth / 2, align: "center" });
  doc.text(clean(company.name).toLocaleUpperCase("vi"), margin + contentWidth / 2, signatureY, { width: contentWidth / 2, align: "center" });
  doc.font("Italic").fontSize(10).fillColor(muted).text("(Ký và ghi rõ họ tên)", margin, signatureY + 20, { width: contentWidth / 2, align: "center" });
  doc.text("(Ký tên, đóng dấu)", margin + contentWidth / 2, signatureY + 20, { width: contentWidth / 2, align: "center" });
};
